...
template <typename F> inline constexpr auto compose(F &&f) noexcept -> F {
...
compose(std::true_type, P, F &&f, Tail &&tail,
        T &&... objs) noexcept(noexcept(f(tail(std::forward<T>(objs)...))))
...
inline constexpr auto
compose(std::false_type, std::true_type, F &&f, Tail &&tail,
        T &&... objs) noexcept(noexcept(f(tail(std::forward<T>(objs))...)))
    -> decltype(f(tail(std::forward<T>(objs))...)) {
  return f(tail(std::forward<T>(objs))...);
}
...
inline constexpr auto compose(F &&f, Fs &&... fs) {
  return
      [f = std::forward<F>(f), tail = compose(std::forward<Fs>(fs)...)](
          auto &&... objs) noexcept(noexcept(detail::
                                                 compose(
                                                     ...
                                                     compose(std::forward<Fs>(
                                                         fs)...),
                                                     LIFT_FWD(objs)...)))
          -> decltype(detail::compose(
              typename std::is_invocable<decltype(
              ...
              std::bool_constant<(
                  std::is_invocable_v<decltype(
                                      ...
                                      decltype(objs)> &&
                  ...)>{},
              f, compose(std::forward<Fs>(fs)...), LIFT_FWD(objs)...)) {
        ...
        return detail::compose(unitail, std::bool_constant<multitail>{}, f,
                               tail, LIFT_FWD(objs)...);
      };
}

template <typename F> inline constexpr auto negate(F &&f) {
  return [f = std::forward<F>(f)](auto &&... obj)
      LIFT_THRICE(!f(LIFT_FWD(obj)...));
}

template <typename T> inline constexpr auto equal(T &&t) {
  return [t = std::forward<T>(t)](const auto &obj) LIFT_THRICE(obj == t);
}

template <typename T> inline constexpr auto not_equal(T &&t) {
  return [t = std::forward<T>(t)](const auto &obj) LIFT_THRICE(obj != t);
}

template <typename T> inline constexpr auto less_than(T &&t) {
  return [t = std::forward<T>(t)](const auto &obj) LIFT_THRICE(obj < t);
}

template <typename T> inline constexpr auto less_equal(T &&t) {
  return [t = std::forward<T>(t)](const auto &obj) LIFT_THRICE(obj <= t);
}

template <typename T> inline constexpr auto greater_than(T &&t) {
  return [t = std::forward<T>(t)](const auto &obj) LIFT_THRICE(obj > t);
}

template <typename T> inline auto constexpr greater_equal(T &&t) {
  return [t = std::forward<T>(t)](const auto &obj) LIFT_THRICE(obj >= t);
}
...
when_all(Fs &fs, std::index_sequence<I...>,
         const T &... t) noexcept(noexcept((std::get<I>(fs)(t...) && ...))) {
  return (std::get<I>(fs)(t...) && ...);
}
} // namespace detail

template <typename... Fs> inline constexpr auto when_all(Fs &&... fs) {
  return [funcs = std::tuple(std::forward<Fs>(fs)...)](
             const auto
                 &... obj) noexcept(noexcept((std::forward<Fs>(fs)(obj...) &&
                                              ...))) -> bool {
    return detail::when_all(funcs, std::index_sequence_for<Fs...>{}, obj...);
  };
}
...
template <typename Fs, std::size_t... I, typename... T>
inline constexpr bool
when_any(Fs &fs, std::index_sequence<I...>,
         const T &... t) noexcept(noexcept((std::get<I>(fs)(t...) || ...))) {
  return (std::get<I>(fs)(t...) || ...);
}
} // namespace detail

template <typename... Fs> inline constexpr auto when_any(Fs &&... fs) {
  return [funcs = std::tuple(std::forward<Fs>(fs)...)](
             const auto
                 &... obj) noexcept(noexcept((std::forward<Fs>(fs)(obj...) ||
                                              ...))) -> bool {
    return detail::when_any(funcs, std::index_sequence_for<Fs...>{}, obj...);
  };
}

template <typename... Fs> inline constexpr auto when_none(Fs &&... fs) {
...
inline constexpr auto if_then(Predicate &&predicate, Action &&action) {
  return [predicate = std::forward<Predicate>(predicate),
          action = std::forward<Action>(action)](
             auto &&... obj) mutable noexcept(noexcept(true ==
                                                       predicate(obj...)) &&
                                              noexcept(action(
                                                  LIFT_FWD(obj)...))) -> void {
    if (predicate(obj...)) {
  ...
  return
      [predicate = std::forward<Predicate>(predicate),
       ...
       f_action = std::forward<FAction>(f_action)](
          ...
          -> std::common_type_t<decltype(t_action(LIFT_FWD(obj)...)),
...
namespace detail {
template <typename Fs, std::size_t... I, typename... T>
inline constexpr void do_all(Fs &fs, std::index_sequence<I...>,
                             const T &... t) {
  ((void)(std::get<I>(fs)(t...)), ...);
}
} // namespace detail

template <typename... Fs> inline constexpr auto do_all(Fs &&... fs) {
  return [funcs = std::tuple(std::forward<Fs>(fs)...)](
             const auto &... obj) mutable noexcept((noexcept(fs(obj...)) &&
                                                    ...)) -> void {
    detail::do_all(funcs, std::index_sequence_for<Fs...>{}, obj...);
  };
}
